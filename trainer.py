...
class Trainer:
    ...
    def train_one_epoch(self):
        ...
        for data in tqdm(self.trainset):
            ...
            wi,wo,imgs,_ = data
            preds,_ = self.net(wi.to(self.device),wo.to(self.device),imgs.to(self.device))
            loss,losses = self.loss(preds,imgs.to(self.device))            
            ...
            if self.steps ==0:
                ...
                running = running*0.75 + loss.item()*0.25
            self.steps+=1
    ...
    def validate(self,epoch,mode,save=False):
        ...
        for data in tqdm(valset):
            with torch.no_grad():
                ...
                preds,props = self.net(wi.to(self.device),wo.to(self.device),imgs.to(self.device))
                ...
                save_path =os.path.join(self.predictions,f'{epoch}_{mode}')
                if not os.path.exists(save_path):
                    os.mkdir(save_path)
                cv2.imwrite(os.path.join(save_path,f'vis_{idx}.png'),cv2.cvtColor(img,cv2.COLOR_RGB2BGR))
                idx+=1
        self.logMemoryUsage()
        for k in metrics:
