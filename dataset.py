...
def gen_img(wi,wo,data,Render,light_intesity):
    length = data.shape[0]
    img = torch.zeros((data.shape[0],3),dtype=data.dtype)
    for idx in range(0,length,1024):
        img[idx:idx+1024,:] = get_rendering(wi,wo,data[idx:idx+1024,:],Render,light_intesity)
    return img
def gen_DiffuseRendering(data,Render):
    ...
    if mutilight:
       wi = wi.unsqueeze_(dim=0)
    img = torch.zeros((data.shape[0],3),dtype=data.dtype)
    light_intensity = torch.rand_like(wi).float()*np.random.randint(1,4)
    for idx in range(0,length,1024):
...
def gen_SpecularRendering(data,Render):    
    ...
    light_intensity = torch.rand_like(wi).float()*np.random.randint(1,4)
    for idx in range(0,length,1024):
        img[idx:idx+1024,:] = get_rendering(wi[idx:idx+1024,:],wo[idx:idx+1024,:],data[idx:idx+1024,:],Render,light_intensity[idx:idx+1024,:])
    return (wi,wo),img,light_intensity
tosave = ['psnr','rre']
class Material(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.mode!='train':
            ...
            return wi,wo,self.light_intesity,img.reshape(h,self.width,3),data
        else:
            ...
            return wi.reshape(h,self.width,3),wo.reshape(h,self.width,3),light_rgb.reshape(h,self.width,3),img.reshape(h,self.width,3),data
